...
public class OAuthUtils {
	...
	public static Token getAccessToken(OAuth2Config oauthDetails) {
		...
		try {
			...
			accessToken = new Token(new Long((Integer) map.get(OAuthConstants.EXPIRES_IN)), (String) map.get(OAuthConstants.TOKEN_TYPE), (String) map.get(OAuthConstants.REFRESH_TOKEN), (String) map.get(OAuthConstants.ACCESS_TOKEN), (String)map.get("roles"),(String)map.get("userName"), (String)map.get("phoneNumber"));
		} catch (ClientProtocolException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IOException e) {
	...
	public static Token refreshAccessToken(Token token) throws Exception {
		OAuth2Client client = new OAuth2Client(GlobalInfo.CoinmarkCalendar.AUTHEN_URL);
		return token.refresh(client);
	}

	public static Token getTokenFromPreference(AppSharePreference appSharePreference) {
		String token = appSharePreference.getAccessToken();
...
//			Token token2 = new Token(Su, tokenType, refreshToken, accessToken, roles)
			return new Token(System.currentTimeMillis(), "", "", token, "", "", "");
		}

	}


	public static Token refreshAccessToken(Token token, OAuth2Config oauthDetails) throws Exception {
		HttpPost post = new HttpPost(oauthDetails.getTokenEndPointUrl());
		String clientId = oauthDetails.getClientId();
		String clientSecret = oauthDetails.getClientSecret();
...
//										+ oauthDetails.getUsername());
						throw new Exception("Could not retrieve access token for user: " + oauthDetails.getUsername());
					}
				}
