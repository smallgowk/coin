...
//		Button imageButton = (Button) frontView
//				.findViewById(R.id.viewToggle);
...
//		});
	...
	/**
	 * Sets animation tempCreatedDate when the user drops the cell
	 * 
	 * @param animationTime
	 *            milliseconds
	 */
	...
	/**
	 * Set thuộc tính để chỉ có một row duy nhất dc mở
	 */
	...
	protected void closeAnimate(int position) {
		View v = swipeListView.getChildAt(
				position - swipeListView.getFirstVisiblePosition());
		if(v != null) {
			closeAnimate(
					v.findViewById(swipeFrontView), position);
		}
	}
...
//			((Button) view
//			.findViewById(R.id.viewToggle)).setBackgroundResource(R.drawable.icon_menu);
		}
	}
...
//		Log.e("SwipeListView", "swap: " + swap + " - swapRight: " + swapRight
//				+ " - position: " + position);
...
//						+ " - swipingRight: " + swipingRight);
			...
			//Dong cac vi tri khac dang mo
			if (singleOpen) {
				int length = opened.size();
				for (int i = 0; i < length; i++) {
					if (i != downPosition && opened.get(i)) {
						closeAnimate(i);
					}
				}
			}
