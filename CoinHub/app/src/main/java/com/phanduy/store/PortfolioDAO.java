...
public class PortfolioDAO {
    ...
    public static final String TABLE_PORT_FOLIO = "PORT_FOLIO";

    private PortfolioDBHelper dbHelper;

    public PortfolioDAO(Context context) {
...
//        int random = new Random().nextInt(100);
        ...
        // Inserting Row
        long rowId = db.insert(TABLE_PORT_FOLIO, null, values);
...
//
    public void delete(String id) {
        ...
        db.delete(TABLE_PORT_FOLIO, KEY_ID + "= ?", new String[] { id });
        db.close(); // Closing database connection
    }
    public void delete(MyCoin myCoin) {
        ...
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        // It's a good practice to use parameter ?, instead of concatenate string
        db.delete(TABLE_PORT_FOLIO, KEY_ID + "= ?", new String[] { myCoin.getId() });
        db.close(); // Closing database connection
    }
//
    public void clearData() {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        // It's a good practice to use parameter ?, instead of concatenate string
        db.delete(TABLE_PORT_FOLIO, null, null);
        db.close(); // Closing database connection
    }
//
    ...
    public void update(SQLiteDatabase db, MyCoin myCoin) {
        ...
        db.update(TABLE_PORT_FOLIO, values, KEY_ID + "= ?", new String[] { myCoin.getId() });
    }
